...
class EditSuaraKebutuhanScreen extends StatefulWidget {
  final String apiBaseUrl;
  final Map<String, dynamic> suaraKebutuhan;

  const EditSuaraKebutuhanScreen({
    Key? key,
  ...
  @override
  State<EditSuaraKebutuhanScreen> createState() => _EditSuaraKebutuhanScreenState();
}

class _EditSuaraKebutuhanScreenState extends State<EditSuaraKebutuhanScreen> {
  ...
  void initState() {
    super.initState();
    _titleController = TextEditingController(text: widget.suaraKebutuhan['title']);
    _descriptionController = TextEditingController(text: widget.suaraKebutuhan['description']);
    _existingImageUrl = widget.suaraKebutuhan['image_url'];

    if (widget.suaraKebutuhan['deadline'] != null && widget.suaraKebutuhan['deadline'] != '') {
      _selectedDeadline = DateTime.parse(widget.suaraKebutuhan['deadline']);
      _deadlineController = TextEditingController(text: DateFormat('dd MMMM yyyy, HH:mm').format(_selectedDeadline!));
    } else {
  ...
  Future<void> _submitForm() async {
    ...
    try {
      var request = http.MultipartRequest(
        'POST',
        Uri.parse('${widget.apiBaseUrl}/update_suara_kebutuhan.php'),
      );

      request.fields['id'] = widget.suaraKebutuhan['id'].toString();
      ...
      if (response.statusCode == 200 && data['status'] == 'success') {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Suara Kebutuhan berhasil diperbarui!')),
        );
        Navigator.pop(context, true);
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Gagal memperbarui: ${data['message'] ?? 'Unknown error'}.')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Edit Suara Kebutuhan'),
      ),
      body: Form(
        key: _formKey,
        child: ListView(
  ...
  Widget _buildImagePicker() {
    ...
    if (_imageFile != null) {
      ...
      imageProvider = NetworkImage('${widget.apiBaseUrl}/$_existingImageUrl');
    }
      ...
      child: DottedBorder(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            color: imageProvider == null ? Colors.grey.shade100 : Colors.transparent,
          ),
